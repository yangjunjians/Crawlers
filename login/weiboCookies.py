...
class WeiboCookies():
    ...
    def prelogin(self):
        ...
        url = 'https://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su={}&rsakt=mod&checkpin=1&client=ssologin.js(v1.4.19)&_={}'.format(quote(self.su),self.get_timestamp()) #注意su要进行quote转码
    ...
    def login(self):
        ...
        html.encoding = 'utf-8'
        cookies = requests.utils.dict_from_cookiejar(self.session.post(url,data=data,allow_redirects=False).cookies)
        return  cookies
...
if __name__ == '__main__':
    username = 'yangjunjians@163.com' # 微博账号
    password = 'yjj7905164' # 微博密码
    weibo = WeiboCookies(username,password)
    cookies = weibo.main()
    print(cookies)
